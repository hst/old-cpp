...
prop_NumLiteral i = v1 == v2
    where
      v1 = run $ eval $ bind "" rootEnv (ENLit i)
      v2 = VNumber i
      types = i :: Int
...
prop_NumNeg = forAll enumber tester
    where
      tester n = i0 == negate i1
          where
            i0 = run $ evalAsNumber $ bind "" rootEnv (ENNeg n)
            i1 = run $ evalAsNumber $ bind "" rootEnv n

prop_NumSum = forAll (two enumber) tester
    where
      tester (n1, n2) = i0 == i1 + i2
          where
            i0 = run $ evalAsNumber $ bind "" rootEnv (ENSum n1 n2)
...
prop_NumDiff = forAll (two enumber) tester
    where
      tester (n1, n2) = i0 == i1 - i2
          where
            i0 = run $ evalAsNumber $ bind "" rootEnv (ENDiff n1 n2)
            i1 = run $ evalAsNumber $ bind "" rootEnv n1
            i2 = run $ evalAsNumber $ bind "" rootEnv n2
...
prop_NumProd = forAll (two enumber) tester
    where
      tester (n1, n2) = i0 == i1 * i2
          where
            i0 = run $ evalAsNumber $ bind "" rootEnv (ENProd n1 n2)
            i1 = run $ evalAsNumber $ bind "" rootEnv n1
            i2 = run $ evalAsNumber $ bind "" rootEnv n2
...
prop_NumLT = forAll (two enumber) tester
    where
      tester (n1, n2) = b0 == (i1 < i2)
          where
            b0 = run $ evalAsBoolean $ bind "" rootEnv (ELT n1 n2)
            i1 = run $ evalAsNumber $ bind "" rootEnv n1
            i2 = run $ evalAsNumber $ bind "" rootEnv n2
...
prop_NumGT = forAll (two enumber) tester
    where
      tester (n1, n2) = b0 == (i1 > i2)
          where
            b0 = run $ evalAsBoolean $ bind "" rootEnv (EGT n1 n2)
            i1 = run $ evalAsNumber $ bind "" rootEnv n1
            i2 = run $ evalAsNumber $ bind "" rootEnv n2
...
prop_NumLTE = forAll (two enumber) tester
    where
      tester (n1, n2) = b0 == (i1 <= i2)
          where
            b0 = run $ evalAsBoolean $ bind "" rootEnv (ELTE n1 n2)
            i1 = run $ evalAsNumber $ bind "" rootEnv n1
            i2 = run $ evalAsNumber $ bind "" rootEnv n2
...
prop_NumGTE = forAll (two enumber) tester
    where
      tester (n1, n2) = b0 == (i1 >= i2)
          where
            b0 = run $ evalAsBoolean $ bind "" rootEnv (EGTE n1 n2)
            i1 = run $ evalAsNumber $ bind "" rootEnv n1
            i2 = run $ evalAsNumber $ bind "" rootEnv n2
